...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
   ...
   <!-- default to false for use by dpe:transform calls to clixform
    ...
    <xsl:template mode="cli-object" match="SSLProxyService">
        <xsl:value-of select="concat($eol, 'sslforwarder ', dpfunc:quoesc(@name), ' ', dpfunc:quoesc(LocalAddress),
                                     ' ', dpfunc:quoesc(LocalPort), ' ', dpfunc:quoesc(RemoteAddress), ' ', dpfunc:quoesc(RemotePort),
                                     ' ', dpfunc:quoesc(SSLProxy))"/>
        <xsl:choose>
          <xsl:when test="not(string(Priority))">
            <xsl:value-of select="concat(' normal')"/>
          </xsl:when>
          <xsl:otherwise>
            <xsl:value-of select="concat(' ', dpfunc:quoesc(Priority))"/>
          </xsl:otherwise>
        ...
        <xsl:choose>
          <xsl:when test="not(string(FrontTimeout))">
            <xsl:value-of select="concat(' 0')"/>
          </xsl:when>
          <xsl:otherwise>
            <xsl:value-of select="concat(' ', dpfunc:quoesc(FrontTimeout))"/>
          </xsl:otherwise>
        </xsl:choose>        
        <xsl:choose>
          <xsl:when test="not(string(BackTimeout))">
            <xsl:value-of select="concat(' 0')"/>
          </xsl:when>
          <xsl:otherwise>
            <xsl:value-of select="concat(' ', dpfunc:quoesc(BackTimeout))"/>
          </xsl:otherwise>
        </xsl:choose>        
        <xsl:choose>
          <xsl:when test="not(string(ConnTimeout))">
            <xsl:value-of select="concat(' 0')"/>
          </xsl:when>
          <xsl:otherwise>
            <xsl:value-of select="concat(' ', dpfunc:quoesc(ConnTimeout))"/>
          </xsl:otherwise>
        </xsl:choose>        
        <xsl:if test="string(ConnLimit)">
          <xsl:value-of select="concat(' ', dpfunc:quoesc(ConnLimit))"/>
        </xsl:if>
        <xsl:if test="string(UserSummary)">
          <xsl:value-of select="concat(' ', dpfunc:quoesc(UserSummary))"/>
        </xsl:if>
        <xsl:value-of select="$eol"/>
    </xsl:template>
  ...
  <!-- ************************************************************ -->
   ...
   <xsl:template mode="HTTPUserAgent" match="*">
    ...
    <xsl:template mode="DNSNameService" match="mAdminState|UserSummary|IPPreference|LoadBalanceAlgorithm|MaxRetries|Timeout">
        <xsl:apply-templates mode="CanonicalProperty" select="."/>
    </xsl:template>
